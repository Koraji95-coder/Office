...
"@
            ...
            try {
                ...
                Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/issues/$($pr.number)/comments" -Method POST -Headers $headers -ContentType "application/json; charset=utf-8" -Body ([System.Text.Encoding]::UTF8.GetBytes($commentBody))
                            ...
                            Write-Host "Failed to post GitHub comment on $repo#$($pr.number): $(# Ensure Ollama has a model loaded
$ollamaRunning = $false
try {
    $ps = Invoke-RestMethod -Uri "http://localhost:11434/api/ps" -ErrorAction Stop
    if ($ps.models.Count -gt 0) { $ollamaRunning = $true }
...
.Exception.Message)"
            ...
            if ($shouldMerge) {
                try {
                    ...
                    Write-Host "Auto-merge failed for $repo#$($pr.number): $(# Ensure Ollama has a model loaded
$ollamaRunning = $false
try {
    $ps = Invoke-RestMethod -Uri "http://localhost:11434/api/ps" -ErrorAction Stop
    if ($ps.models.Count -gt 0) { $ollamaRunning = $true }
