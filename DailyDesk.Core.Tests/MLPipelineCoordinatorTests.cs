...
public sealed class MLPipelineCoordinatorTests
{
    ...
    [Fact]
    public async Task RunMLAnalyticsAsync_FallbackEngine_ReturnsResult()
    {
    ...
    [Fact]
    public async Task RunMLAnalyticsAsync_PersistsResultToStore()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        ...
        var persisted = store.LoadAnalytics();
        Assert.NotNull(persisted);
        Assert.Equal("fallback", persisted.Engine);
    }

    [Fact]
    public async Task RunMLAnalyticsAsync_EmptyAttempts_ReturnsFallbackResult()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        var coordinator = new MLPipelineCoordinator(
        ...
        var result = await coordinator.RunMLAnalyticsAsync([], []);
    ...
    [Fact]
    public async Task RunMLAnalyticsAsync_WithDecisions_ComputesWeakTopics()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        var coordinator = new MLPipelineCoordinator(
        ...
        var attempts = MakeSampleAttempts();
        var result = await coordinator.RunMLAnalyticsAsync(attempts, []);

        // Fallback engine computes topic accuracy from attempts
        Assert.NotNull(result.WeakTopics);
        Assert.NotNull(result.StrongTopics);
        // "grounding" has 50% accuracy (1 correct, 1 wrong) — should be weak
        Assert.Contains(result.WeakTopics, t => t.Topic == "grounding");
        // "protection" has 100% accuracy — should be strong
        Assert.Contains(result.StrongTopics, t => t.Topic == "protection");
    }
    ...
    [Fact]
    public async Task RunMLForecastAsync_FallbackEngine_ReturnsResult()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        var coordinator = new MLPipelineCoordinator(
        ...
        var result = await coordinator.RunMLForecastAsync([]);
    ...
    [Fact]
    public async Task RunMLForecastAsync_PersistsResultToStore()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        var store = new MLResultStore(db);
        ...
        var persisted = store.LoadForecast();
        Assert.NotNull(persisted);
        Assert.Equal("fallback", persisted.Engine);
    }

    [Fact]
    public async Task RunMLForecastAsync_EmptyAttempts_ReturnsResult()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        var coordinator = new MLPipelineCoordinator(
    ...
    [Fact]
    public async Task RunMLAnalyticsAsync_PreCancelledToken_ThrowsOperationCancelled()
    {
        using var tmpDir = new TempDirectory();
        var db = new OfficeDatabase(tmpDir.Path);
        var coordinator = new MLPipelineCoordinator(
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(
            () => coordinator.RunMLAnalyticsAsync([], [], cts.Token));
    }
